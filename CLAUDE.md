...
Access checks use `checkMemberObjects` (transitive membership) with a fallback to `/groups/{id}/owners` for group owners who are not members (HR Administrators and Accounts Manager only — Handbook Editors uses membership check only).
